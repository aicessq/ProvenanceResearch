...
- **Parallel sub-tasks**: Graph nodes create separate agent instances per sub-question/hypothesis, run via `asyncio.gather`, emit events with subtask-specific agent names (`searcher_{id}`, `h_{index}`)
